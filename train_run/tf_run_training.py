...
"""
    ...
    # 将 "python" 替换为 sys.executable (确保使用当前虚拟环境)，并添加 -u (无缓冲) 标志
...
def main():
    ...
    project_root = os.path.dirname(script_dir)  # 向上两级找到项目根目录
    log_dir = os.path.join(project_root, 'train_result')

    # 2. 定义要运行的命令列表 (TF 任务)
    # 参数说明 (基于 tqa_tf_sc.py):
    # -r: 检索方式 (IR, NSP, NN)
    # -s: 保存模型
    # -b 4: 物理 Batch Size (TF任务较轻，可以比MC稍大)
    # -x 64: 最大序列长度 (TF句子较短)
    # -acc 4: 梯度累积步数 (等效 Batch Size = 16)
    commands_config = [
        {
            "cmd": "python tqa_tf_sc.py -r IR -s -b 4 -x 64 -acc 4",
            "logfile": os.path.join(log_dir, "tf_IR_training.txt")
        },
        {
            "cmd": "python tqa_tf_sc.py -r NSP -s -b 4 -x 64 -acc 4",
            "logfile": os.path.join(log_dir, "tf_NSP_training.txt")
        },
        {
            "cmd": "python tqa_tf_sc.py -r NN -s -b 4 -x 64 -acc 4",
            "logfile": os.path.join(log_dir, "tf_NN_training.txt")
        }
    ]

    # 3. 依次执行所有命令
    print(f"[RUNNER] AdaLoGN TF 自动化训练脚本启动...")
    print(f"[RUNNER] 项目根目录: {project_root}")
    ...
    print("\n[RUNNER] 所有 TF (IR, NSP, NN) 训练任务均已成功完成。")
